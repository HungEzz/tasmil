...
export class TwitterSupabase {
  ...
  public async getAllBatches(
    ...
    try {
      if (!this.supabaseClient.hasConfig()) {
        Logger.warn('Supabase not configured. Returning empty batches.');
        return [];
      }
  ...
  public async getTweetsByBatch(batch_id: number): Promise<AiAnalysisBatch[]> {
    try {
      if (!this.supabaseClient.hasConfig()) {
        Logger.warn('Supabase not configured. Returning empty tweets.');
        return [];
      }
      ...
      Logger.error('Error in getTweetsByBatch:', error);
      return [];
    }
  }
  ...
  public async getCursor(): Promise<string> {
    try {
      if (!this.supabaseClient.hasConfig()) {
        Logger.warn('Supabase not configured. Returning cursor 0.');
        return '0';
      }
      ...
      Logger.error('Error in getCursor:', error);
      return '0';
    }
  }
}
